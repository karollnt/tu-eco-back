...
class Solicitudes extends CI_Controller {
    ...
    public function crearSolicitud() {
        ...
        if (strcasecmp($fecha, ":00") == 0 || $fecha == null)
    ...
    public function aceptarPrecio() {
        ...
        $descuento_codigo = 0;
        if (!empty($codigo)) {
            $id_codigo_docente = $this->UsuariosModel->existeCodigoProfesor($codigo);
            if ($id_codigo_docente == -1) {
                $resp = array("error" => html_entity_decode("El codigo del docente No Existe o Ya Fue Utilizado por otro usuario.!!"));
                echo json_encode($resp);
                return;
            }
            $descuento_codigo = $this->UsuariosModel->obtener_valor_paramero("Des_Cod");
        }
        //
        $mensaje = $this->SolicitudesModel->aceptarPrecio($idpreciotrabajo, "WF-" . ($this->random_str(10)), $valor);
        if (strcasecmp($mensaje, "No se pudo actualizar la informaci&oacute;n") == 0)
            $resp = array("error" => html_entity_decode($mensaje));
        else
        //le paso nuevas variables
            $resp = array("msg" => html_entity_decode($mensaje), "nickasistente" => $this->SolicitudesModel->nickAsistenteOferta($idpreciotrabajo), "id_docente_codigo" => $id_codigo_docente, "descuento_codigo" => $descuento_codigo);
        echo json_encode($resp);
    }
    ...
    public function enviarSolucion() {
        ...
        if ($datos2 != null)
            $this->SolicitudesModel->ingresarArchivos($idtrabajo, $datos['idusuario'], $datos2);
    ...
    function aceptarSolucion() {
        ...
        if ($sol) {
            ...
            $mensaje = "Hay problemas para procesar la solicitud";
        $resp = array("msg" => html_entity_decode($mensaje));
        echo json_encode($resp);
    }
    ...
    public function actualizarDireccionCanalChat() {
        ...
        if (strcasecmp($mensaje, "") == 0)
            $resp = array("error" => "No se pudo terminar de procesar la apertura del chat");
        else
            $resp = array("msg" => html_entity_decode($mensaje));
        echo json_encode($resp);
    }
    ...
    public function canalChatTrabajo() {
        ...
        if ($tipo == 1)
            $nick = $this->SolicitudesModel->nickAsistenteTrabajo($idtrabajo);
        else
            $nick = $this->SolicitudesModel->nickUsuarioTrabajo($idtrabajo);
        if (strcasecmp($mensaje, "") == 0)
            $resp = array("error" => "No se pudo terminar de procesar la solicitud");
        else
            $resp = array("msg" => html_entity_decode($mensaje), "nickusr" => $nick);
        echo json_encode($resp);
    }
    ...
    public function procesarPagoBT() {
        ...
        if (strcasecmp($usuario->bt_token, '') != 0) {
            ...
            if (isset($json->error)) {
                ...
                //nuevo bloque de codigo
                if ($codigo != 0 && !empty($codigo)) {
                    $this->UsuariosModel->set_estado_codigo($codigo);
                }
                //
            }
        }
        ...
        if (isset($resultado)) {
            if ($resultado->success === true) {
                ...
                if (isset($resultado->transaction->status)) {
                    if (strcasecmp($resultado->transaction->status, "authorized") == 0 || strcasecmp($resultado->transaction->status, "authorizing") == 0 || strcasecmp($resultado->transaction->status, "settlement_confirmed") == 0 || strcasecmp($resultado->transaction->status, "settled") == 0) {
                        $type = "msg";
                    }
